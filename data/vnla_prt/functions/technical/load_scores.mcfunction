#This jus loads in the scoreboards in case new scores are added or some haven't the first time.

#> Scoreboards to help detect Bedrock and Legacy advancements
# Super Sonic
scoreboard objectives add vp.aviate minecraft.custom:minecraft.aviate_one_cm
scoreboard objectives add vp.elytraSpeed dummy
# Free Diverr and Sleeping with the Fishes
scoreboard objectives add vp.inWater dummy
# Castaway
scoreboard objectives add vp.castaway dummy
# When Pigs Fly and Trampoline
scoreboard objectives add vp.fallDistance minecraft.custom:minecraft.fall_one_cm
# Passing the Time
scoreboard objectives add vp.playTime dummy
# So I Got That Going for Me
scoreboard objectives add vp.llamaCaravan dummy
# Leader of the Pack
scoreboard objectives add vp.tameWolf dummy
# We're Under Attack and Sound the Alarm
scoreboard objectives add vp.raidTrigger minecraft.custom:minecraft.raid_trigger
# On a Rail
scoreboard objectives add vp.minecart minecraft.custom:minecraft.minecart_one_cm
# Map Room
scoreboard objectives add vp.mapRoom dummy
# Ecolocation
scoreboard objectives add vp.echolocation dummy
# Trampoline
scoreboard objectives add vp.trampoline minecraft.custom:minecraft.fall_one_cm
# Master Trader
scoreboard objectives add vp.emerald minecraft.crafted:minecraft.emerald
scoreboard objectives add vp.tradedEmerald dummy
scoreboard objectives add vp.traded minecraft.custom:minecraft.traded_with_villager
# Taste of Your Own Medicine
scoreboard objectives add vp.threwPotion minecraft.used:minecraft.splash_potion

#> Scoreboards for other features to work
# villager age
scoreboard objectives add vp.villagerAge dummy
# setting UUID
scoreboard objectives add vp.uuid dummy
# music disc time
scoreboard objectives add vp.musicDiscTime dummy
# wither health
scoreboard objectives add vp.witherHealth dummy
# wither dash
scoreboard objectives add vp.witherDash dummy
# motion coords
scoreboard objectives add vp.motionX1 dummy
scoreboard objectives add vp.motionZ1 dummy

scoreboard objectives add vp.motionX2 dummy
scoreboard objectives add vp.motionZ2 dummy

# netherrack platform
scoreboard objectives add vp.airCount dummy
scoreboard objectives add vp.magmaCount dummy
scoreboard objectives add vp.lavaCount dummy
scoreboard objectives add vp.countLife dummy